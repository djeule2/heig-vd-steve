...
class CoucheNeige extends Observable implements Observer, Parametres{
	...
	private int tab[] = new int[PG_X+1];
	   ...
	   if(o.getClass() == Flocon.class){
		...
		// Si le flocon se trouve dans la fenêtre
		   if(X_Floc >= 0 && X_Floc <= PG_X){
			   ...
			   if(Y_Floc + hFloc >= PG_Y - tab[X_Floc])
				{
				   int j = 0;
				   // le flocon dégringole sur la face gauche
				   while (tab[X_Floc-j-1]<tab[X_Floc-j] && X_Floc-j >= 1 )
					   j++;
				   tab[X_Floc-j] += hFloc;
				   ...
				   while (tab[X_Floc+k+1]<tab[X_Floc+k] && X_Floc+k <= PG_X-1 )
					   k++;
				   tab[X_Floc+k] += hFloc;
