...
d'attente */
int NB_SIEGES;
/** Variable de type int représentant le nombre de client dans la salle 
d'attente */
int nb_clients_attente = 0;
/** Variable de type int indiquant le temps nécessaire au barbier pour effectuer
		une coupe de cheveux */
int dureeCoupe = 10;
/** Variable de type int indiquant le nombre de client voulant se faire tatouer 
		en attente */
...
/** Sémaphore permettant de faire endormir le barbier lorsque la salle d'attente
*	est vide */
sem_t barbierDort;

/** Sémaphore permettant de représenter une liste des clients présents dans la
* salle d'attente */
sem_t salleAttente;
...
/** sémaphore permettant de faire attendre le client voulant se faire tatoue
 * pendant qu'il se fait tatouer */
sem_t clientTatoue;
...
*				Une deuxième boucle while permet au client de tenter sa chance chez le
*				barbier tant qu'il n'a pas réussi d' obtenir une place dans la salle
*				attente.
*
...
void *Client_tatoue(void *arg){
	...
	while(true){
			printf("le tatoue n° %d réfléchi Ã  un nouveau tatouage\n",numeroClient);
			// Temps aléatoire pour la poussee des cheveux
			// Attente que les cheveux aient poussé
			sleep(dureeAttente);
...
*				que le prochain client le réveil à  l'aide de la sémaphore 
*				'barbierDors'.
*
* Paramètre(s): arg : pointeur passé à  la fonction.
*										  Il est inutilisé, mais permet à  la fonction de
